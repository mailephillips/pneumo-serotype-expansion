...
ds <- (read.csv(file = "carriage_final_Bedouin.csv"))[,2:9]
...
ds1 <- full_join(new2, ds, by="Pnc_Serotype")
ds1[is.na(ds1)] <- 0
...
# -0.6907421 -0.4010870 -0.2128785 -0.01456218 0.3428508
...
dim(prevRsamps)
#[1] 30000   385
write.csv(prevRsamps, "bsamps.bed.prevR.csv")
prevsamps <- as.data.frame(jpos13[[1]][,773:1157])
dim(prevsamps)
#[1] 30000   385
write.csv(prevsamps, "~/Desktop/pneumo/bsamps.bed.prev.csv")
...
# 1458    7222   15933   19448   28561   88836 
...
#beta0s
par(mfrow=c(1,1))
caterplot(jpos13, parms = "beta0.st", labels = order.b0, style = c("plain"), quantiles = list(inner=c(0.025,0.975)), bty="n", lwd = c(1,1), xlab="Log(Relative Risk Ratio)", cex.labels = .5, col="grey40")
...
#beta1s
caterplot(jpos13, parms = "beta1.st", labels = order.b1, style = c("plain"), quantiles = list(inner=c(0.025,0.975)), bty="n", lwd = c(1,1), cex.labels = .5, col="grey40")
title(expression(paste("Model Estimates of ", beta [1][i])))
mtext(expression(bold('Log(Relative Risk Ratio per year)')), side = 1, line = 3)
mtext(expression(bold('Serotype')), side =2, line = 3)
#beta2s
caterplot(jpos13, parms = "beta2.st", labels = order.b2, style = c("plain"), quantiles = list(inner=c(0.025,0.975)), bty="n", lwd = c(1,1), cex.labels = .5, col="grey40")
...
caterplot(jpos13, parms = "exp.beta0.st", labels = order.b0, style = c("plain"), quantiles = list(inner=c(0.025,0.975)), bty="n", lwd = c(1,1), cex.labels = .5, col="grey40")
title(expression(paste("Model Estimates of ", exp(beta [0][j]))))

caterplot(jpos13, parms = "exp.beta1.st", labels = order.b1, style = c("plain"), quantiles = list(inner=c(0.025,0.975)), bty="n", lwd = c(1,1), cex.labels = .5, col="grey40")
title(expression(paste("Model Estimates of ", exp(beta [1][j]))))

caterplot(jpos13, parms = "exp.beta2.st", labels = order.b2, style = c("plain"), quantiles = list(inner=c(0.025,0.975)), bty="n", lwd = c(1,1), cex.labels = .5, col="grey40")
title(expression(paste("Model Estimates of ", exp(beta [2][j]))))



caterplot(jpos13, parms = "prev.ratio.fin", labels = order.prevratio, style = c("plain"), quantiles = list(inner=c(0.025,0.975)), lwd = c(1,1), bty='n', cex.labels = .5, col="grey40")
...
caterplot(jpos13, parms = "log.prev.ratio.fin", labels = order.logprevratio, style = c("plain"), quantiles = list(inner=c(0.025,0.975)), lwd = c(1,1), bty='n', cex.labels = .5, col="grey40")
...
legend(x=1, y=4.5,legend=c('log(prev. ratio): 0','Constant log(prev. ratio): 0.39'),lwd=2, lty=c(2,4), col=c('gray60','black'), cex=.8)
