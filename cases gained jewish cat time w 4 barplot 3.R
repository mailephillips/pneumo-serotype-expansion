...
# prevs <- (read.csv("~/Desktop/pneumo/prevalences y0to6 final NEGwVTs jewish cat time 3.csv"))[,-1]
# prevs <- prevs[-55,]

ds <- (read.csv(file = "~/Desktop/pneumo/multinomialNEGVTs_final_Jewish.csv"))[,2:9]
new.steff <- read.csv("~/Desktop/pneumo/new_st_eff.csv")
new2 <- as.data.frame(new.steff[,2])
names(new2) <- "Serotype"
df1 <- new2


foo <- read.csv("~/Desktop/pneumo/multinomialALLPCV13final.csv")[,3]
foo2 <- as.data.frame((as.character(foo)))
colnames(foo2) <- "st"

# prevRsamps <- read.csv("~/Desktop/pneumo/jsamps.jewish.prevR7.csv")
# prevsamps  <- read.csv("~/Desktop/pneumo/jsamps.jewish.prev7.csv")


#IPD.dist=rpois(IPDN)

#IPD <- (read.csv("~/Desktop/pneumo/ageIPD noVT plus .5 v3.csv"))
IPD <- (read.csv("~/Desktop/pneumo/isr_ipd_maile.csv"))
...
# prevmatxlist <- NULL
# for (i in 1:30000) {
#   prevmatx <- matrix(prevsamps[i,][-1], ncol = 7, byrow = TRUE)
...
obs.IPD_VT_u5 <- obs.IPD_VT_5_17 <- obs.IPD_VT_18_39 <- obs.IPD_VT_40_64 <- obs.IPD_VT_65plus <- obs.IPD_VT_all <- array(NA,dim = c(13,7,30000))
obs.IPD_NVT_u5 <- obs.IPD_NVT_5_17 <- obs.IPD_NVT_18_39 <- obs.IPD_NVT_40_64 <- obs.IPD_NVT_65plus <- obs.IPD_NVT_all <- array(NA,dim = c(61,7,30000))
for (i in 1:30000) {
  for (t in 1:7){
    for (vt in 1:13) {
      obs.IPD_VT_u5[vt,t,i]     <- rpois(n = 1, IPD.VTs_u5[vt,t])
      obs.IPD_VT_5_17[vt,t,i]   <- rpois(n = 1, IPD.VTs_5_17[vt,t])
      obs.IPD_VT_18_39[vt,t,i]  <- rpois(n = 1, IPD.VTs_18_39[vt,t])
      obs.IPD_VT_40_64[vt,t,i]  <- rpois(n = 1, IPD.VTs_40_64[vt,t])
      obs.IPD_VT_65plus[vt,t,i] <- rpois(n = 1, IPD.VTs_65plus[vt,t])
      obs.IPD_VT_all[vt,t,i]    <- rpois(n = 1, IPD.VTs_all[vt,t])
    }
    for (nvt in 1:61) {
      obs.IPD_NVT_u5[nvt,t,i]     <- rpois(n = 1, IPD.NVTs_u5[nvt,t])
      obs.IPD_NVT_5_17[nvt,t,i]   <- rpois(n = 1, IPD.NVTs_5_17[nvt,t])
      obs.IPD_NVT_18_39[nvt,t,i]  <- rpois(n = 1, IPD.NVTs_18_39[nvt,t])
      obs.IPD_NVT_40_64[nvt,t,i]  <- rpois(n = 1, IPD.NVTs_40_64[nvt,t])
      obs.IPD_NVT_65plus[nvt,t,i] <- rpois(n = 1, IPD.NVTs_65plus[nvt,t])
      obs.IPD_NVT_all[nvt,t,i]    <- rpois(n = 1, IPD.NVTs_all[nvt,t])
    }
  }
  print(i)
}
...
for (i in 1:30000){
  for (st in 1:61) {
    for (t in 1:7) {
      exp.cases_u5[st,t,i]     <- obs.IPD_NVT_u5[st,t,i]    /prevratiomatx[st,t,i]
      exp.cases_5_17[st,t,i]   <- obs.IPD_NVT_5_17[st,t,i]  /prevratiomatx[st,t,i]
      exp.cases_18_39[st,t,i]  <- obs.IPD_NVT_18_39[st,t,i] /prevratiomatx[st,t,i]
      exp.cases_40_64[st,t,i]  <- obs.IPD_NVT_40_64[st,t,i] /prevratiomatx[st,t,i]
      exp.cases_65plus[st,t,i] <- obs.IPD_NVT_65plus[st,t,i]/prevratiomatx[st,t,i]
      exp.cases_all[st,t,i]    <- obs.IPD_NVT_all[st,t,i]   /prevratiomatx[st,t,i]
    
    }
  }
}

tot.obs.IPD_VT_u5 <- tot.obs.IPD_VT_5_17 <- tot.obs.IPD_VT_18_39 <- tot.obs.IPD_VT_40_64 <- tot.obs.IPD_VT_65plus <- tot.obs.IPD_VT_all <- array(NA,dim = c(1,7,30000))

tot.obs.IPD_NVT_u5 <- tot.obs.IPD_NVT_5_17 <- tot.obs.IPD_NVT_18_39 <- tot.obs.IPD_NVT_40_64 <- tot.obs.IPD_NVT_65plus <- tot.obs.IPD_NVT_all <- array(NA,dim = c(1,7,30000))

tot.exp.cases_u5 <- tot.exp.cases_5_17 <- tot.exp.cases_18_39 <- tot.exp.cases_40_64 <- tot.exp.cases_65plus <- tot.exp.cases_all <- array(NA,dim = c(1,7,30000))



for (i in 1:30000){
    tot.obs.IPD_VT_u5[,,i]     <- colSums(obs.IPD_VT_u5[,,i])
    tot.obs.IPD_VT_5_17[,,i]   <- colSums(obs.IPD_VT_5_17[,,i])
    tot.obs.IPD_VT_18_39[,,i]  <- colSums(obs.IPD_VT_18_39[,,i])
    tot.obs.IPD_VT_40_64[,,i]  <- colSums(obs.IPD_VT_40_64[,,i])
    tot.obs.IPD_VT_65plus[,,i] <- colSums(obs.IPD_VT_65plus[,,i])
    tot.obs.IPD_VT_all[,,i]    <- colSums(obs.IPD_VT_all[,,i])

    tot.obs.IPD_NVT_u5[,,i]     <- colSums(obs.IPD_NVT_u5[,,i])
    tot.obs.IPD_NVT_5_17[,,i]   <- colSums(obs.IPD_NVT_5_17[,,i])
    tot.obs.IPD_NVT_18_39[,,i]  <- colSums(obs.IPD_NVT_18_39[,,i])
    tot.obs.IPD_NVT_40_64[,,i]  <- colSums(obs.IPD_NVT_40_64[,,i])
    tot.obs.IPD_NVT_65plus[,,i] <- colSums(obs.IPD_NVT_65plus[,,i])
    tot.obs.IPD_NVT_all[,,i]    <- colSums(obs.IPD_NVT_all[,,i])
...
cases.gained_u5     <- tot.obs.IPD_NVT_u5   - tot.exp.cases_u5
cases.gained_5_17   <- tot.obs.IPD_NVT_5_17  - tot.exp.cases_5_17
cases.gained_18_39  <- tot.obs.IPD_NVT_18_39 - tot.exp.cases_18_39
cases.gained_40_64  <- tot.obs.IPD_NVT_40_64 - tot.exp.cases_40_64
cases.gained_65plus <- tot.obs.IPD_NVT_65plus - tot.exp.cases_65plus
cases.gained_all    <- tot.obs.IPD_NVT_all - tot.exp.cases_all
tot.exp.cases_u5[cases.gained_u5<0] <- tot.obs.IPD_NVT_u5[cases.gained_u5<0]
tot.obs.IPD_NVT_5_17[cases.gained_5_17<0] <- tot.exp.cases_18_39[cases.gained_5_17<0]
tot.obs.IPD_NVT_18_39[cases.gained_18_39<0] <- tot.exp.cases_5_17[cases.gained_18_39<0]
tot.obs.IPD_NVT_40_64[cases.gained_40_64<0] <- tot.exp.cases_40_64[cases.gained_40_64<0]
tot.obs.IPD_NVT_65plus[cases.gained_65plus<0] <- tot.exp.cases_65plus[cases.gained_65plus<0]
tot.obs.IPD_NVT_all[cases.gained_all<0] <-tot.exp.cases_all[cases.gained_all<0]
...
cases.gained_u5_st     <- obs.IPD_NVT_u5     - exp.cases_u5
cases.gained_5_17_st   <- obs.IPD_NVT_5_17   - exp.cases_5_17
cases.gained_18_39_st  <- obs.IPD_NVT_18_39  - exp.cases_18_39
cases.gained_40_64_st  <- obs.IPD_NVT_40_64  - exp.cases_40_64
cases.gained_65plus_st <- obs.IPD_NVT_65plus - exp.cases_65plus
cases.gained_all_st    <- obs.IPD_NVT_all    - exp.cases_all
exp.cases_u5[cases.gained_u5_st<0] <- obs.IPD_NVT_u5[cases.gained_u5_st<0]
exp.cases_5_17[cases.gained_5_17_st<0] <- obs.IPD_NVT_5_17[cases.gained_5_17_st<0] 
exp.cases_18_39[cases.gained_18_39_st<0] <- obs.IPD_NVT_18_39[cases.gained_18_39_st<0]
exp.cases_40_64[cases.gained_40_64_st<0] <- obs.IPD_NVT_40_64[cases.gained_40_64_st<0]
exp.cases_65plus[cases.gained_65plus_st<0] <- obs.IPD_NVT_65plus[cases.gained_65plus_st<0]
exp.cases_all[cases.gained_all_st<0] <- obs.IPD_NVT_all[cases.gained_all_st<0]
...
#exp.cases_st.all.st.quants[order(exp.cases_st.all.st.quants[,2], decreasing = T),]


# cases.gained.u5.st.quants <- as.data.frame(cases.gained.u5.st.quants)
# cases.gained.u5.st.quants$wCI <- with(cases.gained.u5.st.quants, sprintf("%.0f (%.0f-%.0f)",median,lCI,uCI))
# cases.gained.u5.st.quants$Serotype <- unlist(foo6)
# colnames(cases.gained.u5.st.quants) <- c("lCI","median","uCI", "medwCI","Serotype")
# cases.gained.u5.st.quants2 <- as.data.frame(left_join(as.data.frame(df1), as.data.frame(cases.gained.u5.st.quants), "Serotype"))
# write.csv(cases.gained.u5.st.quants2,"~/Desktop/pneumo.cases.gained.u5.st.quants.csv")
# 
# cases.gained.5_17.st.quants <- as.data.frame(cases.gained.5_17.st.quants)
# cases.gained.5_17.st.quants$wCI <- with(cases.gained.5_17.st.quants, sprintf("%.0f (%.0f-%.0f)",median,lCI,uCI))
# cases.gained.5_17.st.quants$Serotype <- unlist(foo6)
# colnames(cases.gained.5_17.st.quants) <- c("lCI","median","uCI", "medwCI","Serotype")
...
# write.csv(cases.gained.5_17.st.quants2,"~/Desktop/pneumo.cases.gained.5_17.st.quants.csv")
# 
# cases.gained.18_39.st.quants <- as.data.frame(cases.gained.18_39.st.quants)
# cases.gained.18_39.st.quants$wCI <- with(cases.gained.18_39.st.quants, sprintf("%.0f (%.0f-%.0f)",median,lCI,uCI))
# cases.gained.18_39.st.quants$Serotype <- unlist(foo6)
# colnames(cases.gained.18_39.st.quants) <- c("lCI","median","uCI", "medwCI","Serotype")
...
# write.csv(cases.gained.18_39.st.quants2,"~/Desktop/pneumo.cases.gained.18_39.st.quants.csv")
# 
# cases.gained.40_64.st.quants <- as.data.frame(cases.gained.40_64.st.quants)
# cases.gained.40_64.st.quants$wCI <- with(cases.gained.40_64.st.quants, sprintf("%.0f (%.0f-%.0f)",median,lCI,uCI))
# cases.gained.40_64.st.quants$Serotype <- unlist(foo6)
# colnames(cases.gained.40_64.st.quants) <- c("lCI","median","uCI", "medwCI","Serotype")
...
# write.csv(cases.gained.40_64.st.quants2,"~/Desktop/pneumo.cases.gained.40_64.st.quants.csv")
# 
# cases.gained.65plus.st.quants <- as.data.frame(cases.gained.65plus.st.quants)
# cases.gained.65plus.st.quants$wCI <- with(cases.gained.65plus.st.quants, sprintf("%.0f (%.0f-%.0f)",median,lCI,uCI))
# cases.gained.65plus.st.quants$Serotype <- unlist(foo6)
# colnames(cases.gained.65plus.st.quants) <- c("lCI","median","uCI", "medwCI","Serotype")
...
# write.csv(cases.gained.65plus.st.quants2,"~/Desktop/pneumo.cases.gained.65plus.st.quants.csv")
# 
# cases.gained.all.st.quants <- as.data.frame(cases.gained.all.st.quants)
# cases.gained.all.st.quants$wCI <- with(cases.gained.all.st.quants, sprintf("%.0f (%.0f-%.0f)",median,lCI,uCI))
# cases.gained.all.st.quants$Serotype <- unlist(foo6)
# colnames(cases.gained.all.st.quants) <- c("lCI","median","uCI", "medwCI","Serotype")
# cases.gained.all.st.quants2 <- as.data.frame(left_join(as.data.frame(df1), as.data.frame(cases.gained.all.st.quants), "Serotype"))
# write.csv(cases.gained.all.st.quants2,"~/Desktop/pneumo.cases.gained.all.st.quants.csv")
...
df2 <- df1
#foo2[order(rowSums(cases.gained_all_st), decreasing = T),1]


#cases.gained.y0.quants <- cases.gained.y1.quants <- cases.gained.y2.quants <- cases.gained.y3.quants <- cases.gained.y4.quants <- cases.gained.y5.quants <- cases.gained.y6.quants <- matrix(NA, ncol = 3, nrow=7)
#cases.gained_time_st.all
...
cases.gained.y6.quants <- quantile(cases.gained_time_st.all[1,7,], probs = c(.025,.5,.975))
  ...
  # cases.gained.y0.quants <- quantile(cases.gained_all[1,1,], probs = c(.025,.5,.975))
  # cases.gained.y1.quants <- quantile(cases.gained_all[1,2,], probs = c(.025,.5,.975))
  # cases.gained.y2.quants <- quantile(cases.gained_all[1,3,], probs = c(.025,.5,.975))
  # cases.gained.y3.quants <- quantile(cases.gained_all[1,4,], probs = c(.025,.5,.975))
  # cases.gained.y4.quants <- quantile(cases.gained_all[1,5,], probs = c(.025,.5,.975))
  # cases.gained.y5.quants <- quantile(cases.gained_all[1,6,], probs = c(.025,.5,.975))
  # cases.gained.y6.quants <- quantile(cases.gained_all[1,7,], probs = c(.025,.5,.975))
  ...
  for (t in 1:7){
    ...
    tot.obs.IPD_VT_u5.mcmc[t] <- median(tot.obs.IPD_VT_u5[1,t,])
    tot.obs.IPD_VT_5_17.mcmc[t] <- median(tot.obs.IPD_VT_5_17[1,t,])
    tot.obs.IPD_VT_18_39.mcmc[t] <- median(tot.obs.IPD_VT_18_39[1,t,])
    tot.obs.IPD_VT_40_64.mcmc[t] <- median(tot.obs.IPD_VT_40_64[1,t,])
    tot.obs.IPD_VT_65plus.mcmc[t] <- median(tot.obs.IPD_VT_65plus[1,t,])
    tot.obs.IPD_VT_all.mcmc[t] <- median(tot.obs.IPD_VT_all[1,t,])
    
    tot.exp.cases_u5.mcmc[t] <- median(tot.exp.cases_u5[1,t,])
    tot.exp.cases_5_17.mcmc[t] <- median(tot.exp.cases_5_17[1,t,])
    tot.exp.cases_18_39.mcmc[t] <- median(tot.exp.cases_18_39[1,t,])
    tot.exp.cases_40_64.mcmc[t] <- median(tot.exp.cases_40_64[1,t,])
    tot.exp.cases_65plus.mcmc[t] <- median(tot.exp.cases_65plus[1,t,])
    tot.exp.cases_all.mcmc[t] <- median(tot.exp.cases_all[1,t,])
  }
...
# counts_allu5 <- matrix(c(tot.obs.IPD_VT_u5.mcmc, exp.cases_u5, unexp.cases_u5, cases.gained_u5.mcmc), nrow=4, byrow = T)
# colnames(counts_allu5) <- c("Year 0","Year 1", "Year 2", "Year 3", "Year 4", "Year 5", "Year 6")
# barplot(counts_allu5 ,xlab = '', ylab = '', ylim=c(0,260),
#         col=c("darkblue","red", "darkblue", "red"),density=c(100,100,40,50)) 
# mtext('Under 5 years', side = 3, line = -2, adj = 1, cex=0.9)
# 
# counts_all5_17 <- matrix(c(tot.obs.IPD_VT_5_17.mcmc, exp.cases_5_17, unexp.cases_5_17, cases.gained_5_17.mcmc), nrow=4, byrow = T)
# colnames(counts_all5_17) <- c("Year 0","Year 1", "Year 2", "Year 3", "Year 4", "Year 5", "Year 6")
# barplot(counts_all5_17 ,xlab = '', ylab = '', yaxt = 'n',ylim=c(0,260),
#         col=c("darkblue","red", "darkblue", "red"),density=c(100,100,40,50)) 
# mtext('5-17 years', side = 3, line = -2, adj = 1, cex=0.9)
# 
# counts_all18_39 <- matrix(c(tot.obs.IPD_VT_18_39.mcmc, exp.cases_18_39, unexp.cases_18_39, cases.gained_18_39.mcmc), nrow=4, byrow = T)
# colnames(counts_all18_39) <- c("Year 0","Year 1", "Year 2", "Year 3", "Year 4", "Year 5", "Year 6")
# barplot(counts_all18_39,xlab = '', ylab = '', yaxt='n', ylim=c(0,260),
#         col=c("darkblue","red", "darkblue", "red"),density=c(100,100,40,50)) 
# mtext('18-39 years', side = 3, line = -2, adj = 1, cex=0.9)
# 
# counts_all40_64 <- matrix(c(tot.obs.IPD_VT_40_64.mcmc, exp.cases_40_64, unexp.cases_40_64, cases.gained_40_64.mcmc), nrow=4, byrow = T)
...
# 
# counts_all65plus <- matrix(c(tot.obs.IPD_VT_65plus.mcmc, exp.cases_65plus, unexp.cases_65plus, cases.gained_65plus.mcmc), nrow=4, byrow = T)
# colnames(counts_all65plus) <- c("Year 0","Year 1", "Year 2", "Year 3", "Year 4", "Year 5", "Year 6")
# barplot(counts_all65plus,xlab = '', ylab = '', yaxt='n', ylim=c(0,260),
#         col=c("darkblue","red", "darkblue", "red"),density=c(100,100,40,50)) 
...
# 
# plot.new()
# legend("bottomright",legend = c("Observed VT cases", "Expected NVT cases \nwithout vaccines","Estimated unexplained \nNVT cases", "Estimated NVT cases \ndue to increased carriage"),
#        fill=c("darkblue","red", "darkblue", "red"),density=c(100,100,40,50), cex=1.2, y.intersp=2, bty="n")
# dev.off()
...
for (t in 1:7){
  for (vt in 1:13){
  tot.obs.IPD_VT_st.mcmc[vt,t] <- mean(obs.IPD_VT_all[vt,t,])
  #mean(obs.IPD_VT_all[vt,t,])
  }
  for (nvt in 1:61){
...
for (st in (((16*(n-1))+1:16))){
  ...
  barplot(counts_ALL, xaxt='n',yaxt='n', ylim=c(0,70),
          col=c("red", "red", "grey60"),density=c(100,50,90)) 
  title(st.names[st])
...
n=1
for (st in (((16*(n-1))+1:16))){
  counts_ALL <- matrix(c(tot.exp.cases_st.mcmc[st,], tot.cases.gained_st.mcmc[st,]), nrow=2, byrow = T)
  #counts_ALL[counts_ALL<0] <- 0
  colnames(counts_ALL) <- c("Year 0","Year 1", "Year 2", "Year 3", "Year 4", "Year 5", "Year 6")
  #rownames(counts_ALL) <- c("Observed cases", "Cases gained through serotype replacement")
  barplot(counts_ALL, xaxt='n',yaxt='n',
          col=c("red", "red"),density=c(100,50))
  #legend.text = c("Expected NVT cases without vaccines", "Estimated additional cases due to replacement")) 
  #title("Cases over study period: All individuals")
  title(st.names[st])
  if (st %in% (((16*(n-1))+13:16)))
    axis(1, col = "grey40")
  if (st %in% c(((16*(n-1))+1), ((16*(n-1))+5), ((16*(n-1))+9), ((16*(n-1))+13)))
    axis(2, col = "grey40", at = seq(0,100, 20))
}
mtext("Year", side = 1, outer = TRUE, cex = 0.7, line = 2.2)
mtext("Observed number of NVT IPD cases", side = 2, outer = TRUE, cex = 0.7, line = 2.2)
...
obs.IPD_VT_st_all.mcmc <- matrix(NA, nrow = 30000, ncol = 13)

for (i in 1:30000){
    obs.IPD_VT_st_all.mcmc[i,] <- rowSums(obs.IPD_VT_all[,,i])
    #mean(obs.IPD_VT_all[vt,t,])
}
